...
```bash
npm run share:build
npm run share:release              # lint+build+publish all tag:share (no version bump)

# @hungpvq/shared-store only (version + peer sync + tag shared-store@<ver>)
npm run shared-store:build
npm run shared-store:peers:sync    # rewrite consumers' @hungpvq/shared-store → ^MAJOR.MINOR.0

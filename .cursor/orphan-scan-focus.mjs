...
// SAFE_TO_UNEXPORT: app-local exports used only in same file
const appDirs = [
...
for (const d of appDirs) {
  ...
  for (const file of files) {
    ...
    for (const sym of [...new Set(exports)]) {
      ...
      let crossFileRefs = 0;
      for (const other of allAppFiles) {
...
console.log('\n=== SAFE_TO_UNEXPORT (app-local, same-file only) ===');
for (const u of unexport) console.log(`${u.file} :: ${u.sym}`);

...
describe('attribute-table sort', () => {
  ...
  it('toggles none → asc → desc → none on same key', () => {
    const a = toggleAttributeTableSort(null, 'name');
    expect(a).toEqual({ key: 'name', dir: 'asc' });
    const d = toggleAttributeTableSort(a, 'name');
    expect(d).toEqual({ key: 'name', dir: 'desc' });
    expect(toggleAttributeTableSort(d, 'name')).toBeNull();
  });

  it('multi-sort normal click clears after desc', () => {
    const asc = toggleAttributeTableMultiSort([], 'name');
    expect(asc).toEqual([{ key: 'name', dir: 'asc' }]);
    const desc = toggleAttributeTableMultiSort(asc, 'name');
    expect(desc).toEqual([{ key: 'name', dir: 'desc' }]);
    expect(toggleAttributeTableMultiSort(desc, 'name')).toEqual([]);
  });

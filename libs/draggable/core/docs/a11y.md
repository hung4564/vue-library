...
| Popup / float | `dialog` (non-modal) | no | closes if focus in panel | `focusFirst` | no | no (map stays usable) |
...
Icon-only chrome (`MapButton`): native `disabled` + descriptive `aria-label`; expand toggles also set `aria-expanded` (and `aria-controls` where a content host id exists). Decorative header rules use `aria-hidden="true"`.
...
- Does **not** use `setModalSiblingsInert` or mandatory tab trap

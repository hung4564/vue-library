...
Icon-only chrome (`DragButton`): native `disabled` + descriptive `aria-label`; expand toggles also set `aria-expanded` (and `aria-controls` where a content host id exists). Decorative header rules use `aria-hidden="true"`.
